...
create_symlinks() {
    ...
    while IFS=: read -r source target || [ -n "$source" ]; do
        ...
        if [ ! -e "$source" ]; then
            _error "Error: Source file '$source' not found. Skipping link creation for '$target'."
            continue
        fi

        # Check if the symbolic link already exists
        if [ -L "$target" ]; then
            _warning "Symbolic link already exists: $target"
        elif [ -f "$target" ]; then
            _warning "File already exists: $target"
        else
            ...
            if [ ! -d "$target_dir" ]; then
                mkdir -p "$target_dir"
                _info "Created directory: $target_dir"
            fi

            # Create the symbolic link
            ln -s "$source" "$target"
            _success "Created symbolic link: $target"
        fi
    done <"$CONFIG_FILE"
}

delete_symlinks() {
    _info "Deleting symbolic links..."
...
if [ "$(basename "$0")" = "$(basename "${BASH_SOURCE[0]}")" ]; then
    ...
    "--help")
        ...
        ;;
    *)
        _error "Error: Unknown argument '$1'"
        _error "Usage: $0 [--create | --delete [--include-files] | --help]"
        exit 1
        ;;
    esac
fi
